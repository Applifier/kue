...
function getSearch() {
  ...
  reds.createClient = function() {
    var client = redisFactory.createClient("search");
    return client;
  };
  ...
  return function(err, ids){
    ...
    ids.forEach(function(id){
      exports.get(id, function(err, job){
        ...
        jobs[job.id] = job;
        if (--pending === 0) {
          fn(null, 'desc' == order ? map(jobs, ids).reverse() : map(jobs, ids));
        }
...
exports.range = function(from, to, order, fn){
  this.client.zrange('q:jobs', from, to, get(fn, order));
};
...
exports.rangeByState = function(state, from, to, order, fn){
  this.client.zrange('q:jobs:' + state, from, to, get(fn, order));
};
...
exports.rangeByType = function(type, state, from, to, order, fn){
  this.client.zrange('q:jobs:' + type + ':' + state, from, to, get(fn, order));
};
...
exports.log = function(id, fn){
  this.client.lrange('q:job:' + id + ':log', 0, -1, fn);
};
...
/**
 * Send arbitrary events back to the job sender
 * @param name
 * @param args
 */
Job.prototype.emitRemote = function(name, args) {
  this.queue.events.emit(this.id, name, args);
};
...
Job.prototype.progress = function(complete, total){
  ...
  this.set('progress', n);
  this.queue.events.emit(this.id, 'progress', n);
  return this;
};
...
Job.prototype.save = function (fn) {
  var max = this._max_attempts
      , self = this;

  fn = fn || noop;

  // Lazily subscribe to events
  this.queue.subscribeToEvents(function () {
    // update
    if (this.id) return self.update(fn);
    ...
    self.client.incr('q:ids', function (err, id) {
      ...
      if (max) self.client.hset(key, 'max_attempts', max);
      self.client.sadd('q:job:types', self.type);
