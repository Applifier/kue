...
function Worker(queue, type) {
  this.queue = queue;
  this.type = type;
  this.client = Worker.client || (Worker.client = redis.createClient("Worker " + type));
  this.interval = 1000;
}
...
Worker.prototype.getJob = function(fn){
  ...
  var client = clients[self.type] || (clients[self.type] = redis.createClient("getJob " + self.type));
